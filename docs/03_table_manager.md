...
> **Hinweis:**
> In REDAXO 5 waren Teile von YForm eigene Plugins, die man einzeln aktivieren konnte.
> In REDAXO 6 gibt es kein Plugin-Konzept mehr — alles Folgende ist fester Bestandteil von YForm.
...
```php
$dataset = Yakamara\YForm\Manager\Dataset::create('rex_my_yform_table');
$yform = $dataset->getForm();
$yform->setObjectparams('form_action', Redaxo\Core\Filesystem\Url::article(Redaxo\Core\Content\Article::getCurrentId()));
...
Daten im Table Manager werden in der SQL-Datenbank abgelegt, die bei der Redaxo-Installation angegeben wurde. Die einfachste Möglichkeit ist daher, über das [Redaxo\Core\Database\Sql-Objekt](https://github.com/redaxo/redaxo/wiki/Aenderungen-in-REDAXO-5#Redaxo\Core\Database\Sql) die Daten auszulesen.
...
Außerdem lässt sich pro Tabelle eine Historie aktivieren, mit der Änderungen an Datensätzen nachverfolgt werden können.

### Für Entwickler: Table Manager erweitern

Es ist möglich, eigene Feldtypen zu definieren und dem Table Manager hinzuzufügen. Wie das geht, steht im Kapitel [Formbuilder](?page=yform/docs&mdfile=07_formbuilder) unter „Eigene Feldtypen“.
...
> Tipp: Zum konvertieren des MySQL-Date-Formats in PHP `date(Redaxo\Core\Database\Sql::FORMAT_DATETIME, strtotime($datetime))` verwenden
...
```php
<?php
if ($this->params['main_id'] > 0 && Redaxo\Core\Core::isBackend()) {
...
Diese Funktion z. B. in `Project::boot()` (`src/Project.php`) registrieren:
...
* @param $that \Yakamara\YForm\Validate\CustomFunction
* @param $Objects array[]
* @return bool
 */
...
{
	Redaxo\Core\ExtensionPoint\Extension::register("YFORM_DATA_LIST", function( $ep ) {  
...
Die Snippets kommen am besten in `Project::boot()` (`src/Project.php`).
        ...
        if ($ep->getParam('table')->getTableName()=="rex_tabelle") { // Tabellenname anpassen
            ...
            $list->setColumnFormat(
                ...
                function ($a) {
                    ...
                    $_csrf_key = Yakamara\YForm\Manager\Table\Table::get('rex_tabelle')->getCSRFKey();
                    ...
                    return '<a href="'.Redaxo\Core\Filesystem\Url::backendPage('yform/manager/data_edit', $params) .'">'. $a['value'].'</a>';
                }
            );
        }
    });
...
if (Redaxo\Core\Core::isBackend() && Redaxo\Core\Http\Request::request('table_name') == 'rex_test') {
...
```php
<?php
if(Redaxo\Core\Core::isBackend() && Redaxo\Core\Addon\Addon::get('yform')?->isActivated() &&
   rex_be_controller::getCurrentPage() == 'yform/manager/data_edit' && Redaxo\Core\Http\Request::request('table_name') == '<TABLE_NAME>') {
	Redaxo\Core\ExtensionPoint\Extension::register('YFORM_DATA_LIST_SQL', function(Redaxo\Core\ExtensionPoint\ExtensionPoint $ep){
		...
		if(Redaxo\Core\Http\Request::request("sort") != "" && Redaxo\Core\Http\Request::request("sort") != $sortField) {
			return;
		}
	...
	}, Redaxo\Core\ExtensionPoint\Extension::LATE);
}
```
...
```php
<?php
Redaxo\Core\ExtensionPoint\Extension::register('YFORM_DATA_LIST_SQL', function ($ep) {
  ...
  if ($params['table'] == Redaxo\Core\Core::getTable('my_table')) {
...
$query = 'SELECT * FROM ' . Redaxo\Core\Core::getTable('news_beitrag') . ' JOIN ' . Redaxo\Core\Core::getTable('news_tag') . ' ON ' . Redaxo\Core\Core::getTable('news_beitrag') . '.id_tag = ' . Redaxo\Core\Core::getTable('news_tag') . '.id ';
...
<?php
$sql = Redaxo\Core\Database\Sql::factory()
$query = 'SELECT ' . Redaxo\Core\Core::getTable('news_beitrag') . '.*, GROUP_CONCAT(' . Redaxo\Core\Core::getTable('news_tag') . '.name SEPARATOR ",") AS tags ';
$query.= 'FROM ' . Redaxo\Core\Core::getTable('news_beitrag') . ' ';
$query.= 'JOIN ' . Redaxo\Core\Core::getTable('news_tag_beitrag') . ' ON ' . Redaxo\Core\Core::getTable('news_beitrag') . '.id = ' . Redaxo\Core\Core::getTable('news_tag_beitrag') . '.id_beitrag ';
$query.= 'JOIN ' . Redaxo\Core\Core::getTable('news_tag') . ' ON ' . Redaxo\Core\Core::getTable('news_tag') . '.id = ' . Redaxo\Core\Core::getTable('news_tag_beitrag') . '.id_tag ';
$query.= 'GROUP BY ' . Redaxo\Core\Core::getTable('news_beitrag') . '.id';
## $sql->setQuery($query);
$rows = $sql->getArray($query);
foreach($rows as $row) {
    echo '<h1>' . $row['titel'] . '</h1>';
...
Der beste Weg ist es, sich in `Project::boot()` (`src/Project.php`) über einen Extension Point (EP) an die Listenausgabe anzudocken.

```php
Redaxo\Core\ExtensionPoint\Extension::register('YFORM_DATA_LIST', static function ($ep) {
    /** @var Yakamara\YForm\ListView\YList $list */
    $list = $ep->getSubject();
    $params = $ep->getParams();
        ...
        if ('value' == $field->getType() && in_array($field->getTypeName(), $textFields, true)) {
            $list->setColumnFormat(
                ...
                static function ($params) {
                    $value = Redaxo\Core\View\escape($params['value']);
                    // Hier die Veränderungen einbauen
                    return Redaxo\Core\View\escape($value);
                }
            );
        }
    }
