...
### Wo die Formular-Definition steht

Eine Formular-Definition ist reiner Text in der unten beschriebenen Pipe-Notation. Sie kann überall
stehen, wo PHP-Code läuft — am häufigsten in einem Modul, damit Redakteure das Formular in einem
Artikel platzieren können.

REDAXO 5 installierte dafür auf Knopfdruck ein fertiges Modul (*Setup › Modul „YForm Formbuilder"
installieren*). In REDAXO 6 gibt es keine Modultabelle mehr; ein Modul ist eine PHP-Klasse im Projekt.
Das Kapitel [Modul erstellen](?page=yform/docs&mdfile=05_modul) enthält ein vollständiges Modul, das
genau diese Aufgabe übernimmt — inklusive Eingabefeld für die Definition.

Ohne Modul geht es genauso, etwa direkt in einem Template oder einer eigenen Klasse:
...
In die Formular-Definition trägt man Values, Validierungen und Aktionen zeilenweise ein.
...
> **Tipp:**
> Wer sich die `output()`-Methode eines Formular-Moduls ansieht, versteht das Prinzip von YForm
> schneller. Der Kern sind vier Zeilen:
...
$yform = new YForm();
$yform->setObjectparams('form_action', Url::article($slice->articleId, Language::getCurrentId()));
$yform->setFormData($definition);
echo $yform->getForm();
```

Zunächst wird ein neues `YForm`-Objekt erzeugt. Dann wird als Ziel des Formularversands der aktuelle
Artikel in der aktuellen Sprache gesetzt — sonst sendet YForm an `index.php` und Fehlermeldungen
gehen verloren. Schließlich werden die Felddefinitionen übergeben und das Formular ausgegeben.

Die vordefinierten Aktionen (E-Mail senden, in die Datenbank schreiben, Erfolgsmeldung ausgeben) sind
nichts anderes als Action-Felder, die man entweder in die Definition schreibt oder per
`setActionField()` ergänzt. Ein Modul erleichtert also nur die redaktionelle Bedienung — auslösen
lässt sich alles genauso aus eigenem PHP-Code.

> **REDAXO 5:** Dort stand in der Modulausgabe `'REX_VALUE[3]'` als Platzhalter für den Inhalt des
> Eingabefelds. REDAXO 6 kennt keine `REX_VALUE`-Platzhalter mehr; in einer Modulklasse liest man den
> Wert mit `$slice->getValue(3)`.
...
Wenn man den Wert hier auf `plain` setzt, werden die Feld-Label nicht als HTML interpretiert, sondern mit `Redaxo\Core\View\escape` und `nl2br` maskiert.  
Default ist `html`.
...
Das HTML rund um die Felder kommt aus **Fragmenten**, nicht mehr aus Template-Themes. Der Objparam
`form_ytemplate` ist damit entfallen — welches Markup greift, hängt nur noch davon ab, welche
Fragmente registriert sind.

Ausgeliefert wird Bootstrap-3-Markup unter `fragments/yform/value/backend/*` mit einer
Frontend-Variante daneben, die zunächst dorthin weiterreicht. Wie man einzelne Fragmente
überschreibt, steht unter
[Eigene Formular-Templates verwenden](?page=yform/docs&mdfile=06_demos).
...
```php
<?php
<?php mit Redaxo\Core\Filesystem\Url::article() auf die Artikel-ID 5
$yform->setObjectparams('form_action',Redaxo\Core\Filesystem\Url::article(5));
```
...
<?php
$yform = new Yakamara\YForm\YForm();
$yform->setObjectparams('form_action', Redaxo\Core\Filesystem\Url::article(REX_ARTICLE_ID,REX_CLANG_ID));
...
7. Select aus SQL, gruppiert mit Leeroption und bevorzugter Auswahl
        ...
        choice|artikel|Artikel|SELECT name label, id value, catname FROM Redaxo\Core\Content\Article ORDER BY catname|0|0||catname|8,5|--- bitte auswählen ---
...
>
> **Wichtig:** Der Operator beschreibt, wann die Eingabe **falsch** ist — nicht wann sie
> richtig ist. `!=` meldet also einen Fehler, *wenn die Felder verschieden sind*, und erzwingt
> damit Gleichheit. Genau in dieser Richtung sind die Beispiele oben zu lesen.
...
> **Hinweis:** Mögliche Vergleichs-Operatoren sind `!=`, `<`, `>`, `==`, `>=` und `<=`
>
> **Wichtig:** Auch hier ist der Operator die Fehlerbedingung: `<` meldet einen Fehler,
> *wenn der Wert kleiner* als der Vergleichswert ist.
...
> **Hinweis:** Ohne den letzten Parameter prüft `type` **auch leere Eingaben** — ein optionales
> E-Mail-Feld meldet dann „keine gültige E-Mail", wenn es leer bleibt. Für optionale Felder daher
> `1` anhängen:
>
>     validate|type|email|email|Keine gültige E-Mail-Adresse|1
...
Überprüft, ob ein Datensatz mit einem bestimmten Feld-Wert bereits in einer Datenbank-Tabelle vorhanden ist.
	...
	// Im YForm-Formbuilder
	text|email|E-Mail|
	validate|unique|email|Ein User mit dieser E-Mail-Adresse existiert schon!|Redaxo\Core\Security\User

```php
<?php
$yform->setValueField('text', array("email","E-Mail"));
$yform->setValidateField('unique', array("email", "Ein User mit dieser E-Mail-Adresse existiert schon!","Redaxo\Core\Security\User"));
```
...
// mit Übergabe von URL-Parameter(n):
$yform->setActionField('redirect', [Redaxo\Core\Filesystem\Url::article(32, Redaxo\Core\Language\Language::getCurrentId(), ['mein_parameter' => 'mein_wert'])]);
// oder
...
### Eigene Values, Validates und Actions

Ein eigener Feldtyp ist eine Klasse, die von `AbstractValue`, `AbstractValidate` oder `AbstractAction`
erbt und mit dem passenden Attribut markiert ist. Das Attribut legt den **Typnamen** fest — also den
Namen, den man in der Pipe-Notation und in `rex_yform_field.type_name` verwendet.

```php
<?php

namespace Project\YForm\Value;

use Override;
use Yakamara\YForm\Attribute\AsValue;
use Yakamara\YForm\Value\AbstractValue;

#[AsValue('sternebewertung')]
final class SterneBewertung extends AbstractValue
{
    #[Override]
    public function enterObject(): void
    {
        ...
        if ($this->saveInDb()) {
            $this->params['value_pool']['sql'][$this->getName()] = (int) $this->getValue();
        }
    ...
    {
        return 'sternebewertung|name|label|[1-5]';
    }
...
Danach steht `sternebewertung` sowohl in der Pipe-Notation als auch in der Feldauswahl des Table
Managers zur Verfügung. Die Attribute für die anderen beiden Sorten heißen `#[AsValidate('…')]` und
`#[AsAction('…')]`.

Die Klasse muss lediglich vom Autoloader gefunden werden, also unterhalb eines PSR-4-Verzeichnisses
des Projekts oder eines AddOns liegen. Nach dem Anlegen einmal:

```bash
php bin/console cache:clear
```

Der Cache ist nötig, weil die Zuordnung Typname → Klasse zwischengespeichert wird.

> **REDAXO 5:** Dort ergab sich der Typname aus dem Klassennamen — eine Klasse
> `rex_yform_value_sternebewertung` wurde automatisch als Typ `sternebewertung` erkannt. Diese
> Konvention gibt es nicht mehr, weil REDAXO 6 keinen Klassenindex besitzt. Bestehende Klassen nach
> alter Namenskonvention funktionieren aber weiterhin: findet YForm keinen Typ über die Attribute,
> sucht es zusätzlich nach `rex_yform_value_<typname>`.

### Ein eigenes Framework für den Formularcode verwenden

Standardmäßig gibt YForm Bootstrap-3-Markup aus. Das Markup steckt in Fragmenten, die sich einzeln
überschreiben lassen — man kopiert nur die Dateien, die anders aussehen sollen.

Dazu in `Project::boot()` (`src/Project.php`) das eigene Fragment-Verzeichnis anmelden:

```php
use Redaxo\Core\Filesystem\Path;
use Redaxo\Core\View\Fragment;

Fragment::addDirectory(Path::base('fragments/'));
```

Dann die gewünschten Dateien aus `fragments/yform/` des Composer-Pakets `yakamara/yform` in das
eigene Verzeichnis übernehmen — unter demselben Pfad — und anpassen. Für ein anderes Markup nur im
Frontend genügt `fragments/yform/value/frontend/text.php`; das Backend bleibt dann unverändert.

> **Tipp:** Es genügt, einzelne Dateien abzulegen. Alles, was man nicht überschreibt, kommt
> weiterhin aus yform.
...
In `Project::boot()` (`src/Project.php`) folgenden Code aufnehmen, um am Beispiel des `ycom`-Addons die E-Mail-Adresse zu verlinken. Es muss ein Feld `email` in der Tabelle existieren.
...
Redaxo\Core\ExtensionPoint\Extension::register('YFORM_DATA_LIST','my_rex_list_tweaks');

function my_rex_list_tweaks($ep)

{
 $list = $ep->getSubject();
//if ($ep->getParam('table')->getTableName() == Redaxo\Core\Core::getTable('ycom_user')) { // optional, wenn nur eine bestimmte Tabelle verändert werden soll
