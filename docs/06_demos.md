...
```php
<?php
$yform->setValueField('php', array('php_attach', 'Datei anhängen', '<?php $this->params[\'value_pool\'][\'email_attachments\'][] = [\'agb.pdf\', Redaxo\Core\Filesystem\Path::media(\'agb.pdf\')]; ?>'));
```

#### Pipe-Schreibweise
```
php|php_attach|Datei anhängen|<?php $this->params[\'value_pool\'][\'email_attachments\'][] = [\'agb.pdf\', Redaxo\Core\Filesystem\Path::media(\'agb.pdf\')]; ?>
```
...
foreach ($dateiliste as $file) {
    $yform->setValueField('php', array('php_attach', 'Datei anhängen', '<?php $this->params[\'value_pool\'][\'email_attachments\'][] = [\''.$file.'\', Redaxo\Core\Filesystem\Path::media(\''.$file.'\')]; ?>'));
}
```
...
> Tipp: Die Funktion kann z. B. in `Project::boot()` (`src/Project.php`) registriert werden.
...
## Eigene Formular-Templates verwenden

Das Markup der Formularfelder liegt in **Fragmenten**. REDAXO 5 nutzte dafür ein eigenes
`ytemplates`-Verzeichnis mit `addTemplatePath()` und Template-Gruppen (`form_ytemplate`) — beides
gibt es in REDAXO 6 nicht mehr. Stattdessen greift der normale Fragment-Mechanismus des Cores.
...
fragments/yform/
  form/
    backend/form.php          das <form>-Element
    backend/errors.php        der Fehlerblock über dem Formular
    frontend/form.php         gibt an die Backend-Variante weiter
    frontend/errors.php
  value/
    backend/text.php          ein Fragment je Feldtyp
    backend/textarea.php
    backend/choice.select.php
    …
    frontend/text.php         gibt an die Backend-Variante weiter
    …
```

Backend und Frontend sind getrennt, weil ein Eingabeformular im Backend anders aussehen darf als
eines auf der Website. Ausgeliefert enthält jede Frontend-Datei nur eine Weitergabe an ihr
Backend-Gegenstück — solange man nichts ändert, sieht also beides gleich aus:
...
### Ein Fragment überschreiben

Man legt dieselbe Datei im eigenen Fragment-Verzeichnis an; das eigene gewinnt, weil es später
registriert wurde. Für ein AddOn oder die Projektklasse:

```php
<?php

// src/Project.php, in boot()
...
Danach überschreibt `fragments/yform/value/frontend/text.php` im Projekt das mitgelieferte
Frontend-Fragment für `text` — und nur dieses. Alle übrigen Felder bleiben unverändert.

### Variablen im Fragment

Jedes Fragment beginnt mit `extract($this->getVariables())`. Danach stehen zur Verfügung:

Variable | in | Bedeutung
-------- | -- | ---------
`$yfield` | Value-Fragmente | das Feld-Objekt, z. B. `$yfield->getLabel()`, `$yfield->getFieldName()`
`$yform` | Form-Fragmente | das Formular-Objekt, z. B. `$yform->objparams['form_action']`
weitere | beide | was die aufrufende Klasse mitgibt, z. B. `$type` und `$prepend` bei `text`

`$this` ist das Fragment selbst — nicht das Feld. Über `$this->delegate('…')` reicht man an ein
anderes Fragment weiter.

> **Hinweis**
> Ein Fragment ohne Frontend-Variante fällt automatisch auf die Backend-Datei zurück. Die
> mitgelieferten Frontend-Dateien existieren trotzdem alle, damit man sofort sieht, wo man ansetzen
> kann.
...
```php
    <div class="modul-wrapper">       
        <?php
$key = Redaxo\Core\Http\Request::get('key', 'string', 0);
$token = Redaxo\Core\Http\Request::get('token', 'string', 0);

if($key && $token) {
    $member = current(Redaxo\Core\Database\Sql::factory()->setDebug(0)->getArray('SELECT * FROM rex_yf_member WHERE key = ? AND token = ? AND status = 0 LIMIT 1', array($key, $token)));

    if(count($member)) {
        if($member['token'] == $token) {
            Redaxo\Core\Database\Sql::factory()->setDebug(0)->setQuery('UPDATE rex_yf_member SET token = NULL, key = NULL, status = 1, updatedate = NOW() WHERE key = ?', array($key));
