...
```php
<?php
$items = Yakamara\YForm\Manager\Table\Table::get('rex_my_table')->query()->find();
dump($items);
```
...
```php
<?php
class MyTable extends \Yakamara\YForm\Manager\Dataset
{
}
```
...
Jetzt muss die erstellte Klasse noch registiert werden. Dazu ergänzt man die `boot()`-Methode der Projektklasse (`src/Project.php`, erbt von `Redaxo\Core\AbstractProject`). Sie läuft bei jedem Request, nachdem Core und AddOns gebootet haben.

```php
<?php
Yakamara\YForm\Manager\Dataset::setModelClass('rex_my_table', MyTable::class);
```
...
```php
<?php
$table = Yakamara\YForm\Manager\Table\Table::get('rex_data_product');
...
```php
<?php
    $post = Yakamara\YForm\Manager\Dataset::get($id, 'rex_blog_post');
?>
<article>
    <h1><?= $post->title ?></h1>
    <p><?= $post->text ?></p>
...
$dataset = Yakamara\YForm\Manager\Dataset::get(2,'tabelle');
// Formular auslesen
$yform = $dataset->getForm();
// Parameter festlegen
$yform->setObjectparams('form_method','get');
// Ziel des Formulars, sonst erhält man nur Index.php ...
$yform->setObjectparams('form_action',Redaxo\Core\Filesystem\Url::article(REX_ARTICLE_ID));
...
```php
<?php
$post = Yakamara\YForm\Manager\Dataset::create('rex_blog_post');
$post->title = 'REDAXO-Tag in Wackershofen (am Grundbach)';
$post->text = '...';
...
$dataset = Yakamara\YForm\Manager\Dataset::create('tabelle');
// Formular auslesen
$yform = $dataset->getForm();
// Parameter festlegen
$yform->setObjectparams('form_action',Redaxo\Core\Filesystem\Url::article(REX_ARTICLE_ID));
// Ziel des Formulars, sonst erhält man nur Index.php ...
$yform->setObjectparams('form_action',Redaxo\Core\Filesystem\Url::article());
$yform->setActionField('showtext',array('','Gespeichert'));
echo $dataset->executeForm($yform);
} ?>
```
...
Yakamara\YForm\Manager\Dataset::setModelClass(
    'rex_blog_author',
...
<?php
// lib/post.php
class rex_blog_post extends Yakamara\YForm\Manager\Dataset
{
...
<?php
// lib/author.php
class rex_blog_author extends Yakamara\YForm\Manager\Dataset
{
...
$table = Yakamara\YForm\Manager\Table\Table::get('rex_table_name');
$ergebnisse = $table->query()
    ->paginate($pager);
$fragment = new rex_fragment();
$fragment->setVar('urlprovider', Redaxo\Core\Content\Article::getCurrent());
...
Wichtig ist nur der Part mit `Redaxo\Core\Database\Sql`
...
$query
    ...
    ->where('t.status', '1')
    ->orderBy('t.name')
$items = Redaxo\Core\Database\Sql::factory()->setDebug()->getArray($query->getQuery(), $query->getParams());
$items = $query->find();
```
...
In `src/Manager/Dataset.php` die Variable `private static $debug = false;` auf `true` setzen
...
$yform->setObjectparams('form_method','get');
// Ziel des Formulars, sonst erhält man nur Index.php ...
$yform->setObjectparams('form_action',Redaxo\Core\Filesystem\Url::article(13));
// Sollen die Daten des Datensatzes ausgelesen werden? (true = ja , false = nein) 
$yform->setObjectparams("getdata",true);
echo $dataset->executeForm($yform);
} ?>
...
```php
<?php
class rex_data_mydata extends Yakamara\YForm\Manager\Dataset
{
...
Model-Klasse in `Project::boot()` (`src/Project.php`) registrieren

```php
<?php
Yakamara\YForm\Manager\Dataset::setModelClass('rex_my_table', rex_data_mydata::class);
```
