...
$yform->setObjectparams('form_action',Redaxo\Core\Content\Article::getCurrent()->getUrl());
...
if($form) { // Wenn das Formular nicht abgesendet wurde
	...
	if ($yform_email_template = Yakamara\YForm\Email\Template::getTemplate($yform_email_template_key)) {

	    if ($debug) {
	        echo '<hr /><pre>'; var_dump($yform_email_template); echo '</pre><hr />';
	    }
	    $yform_email_template = Yakamara\YForm\Email\Template::replaceVars($yform_email_template, $values);
	    ...
	    if (!Yakamara\YForm\Email\Template::sendMail($yform_email_template, $yform_email_template_key)) {
	    ...
	    if ($debug) {echo '<p>YForm E-Mail-Template "'.Redaxo\Core\View\escape($yform_email_template_key).'" wurde nicht gefunden.'; }
	}
}
?>
```
...
Dieser Code entspricht dem, was die Action `tpl2email` (`src/Action/Tpl2Email.php`) intern tut.
