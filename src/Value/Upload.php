...
class Upload extends AbstractValue
{
    ...
    public function enterObject()
    {
        ...
        $temp_folder = Path::addonData('yform', 'upload/temp');

        Dir::create($upload_folder);
        ...
        if (!isset($configuration['messages']['min_error'])) {
            $configuration['messages']['min_error'] = isset($messages[0]) ? I18n::translate($messages[0]) : 'min-error-msg';
        }

        if (!isset($configuration['messages']['max_error'])) {
            $configuration['messages']['max_error'] = isset($messages[1]) ? I18n::translate($messages[1]) : 'max-error-msg';
        }

        if (!isset($configuration['messages']['type_error'])) {
            $configuration['messages']['type_error'] = isset($messages[2]) ? I18n::translate($messages[2]) : 'type-error-msg';
        }

        if (!isset($configuration['messages']['empty_error'])) {
            $configuration['messages']['empty_error'] = isset($messages[3]) ? I18n::translate($messages[3]) : 'empty-error-msg';
        }

        if (!isset($configuration['messages']['system_error'])) {
            $configuration['messages']['system_error'] = isset($messages[5]) ? I18n::translate($messages[5]) : 'system_error-msg';
        }

        if (!isset($configuration['messages']['type_multiple_error'])) {
            $configuration['messages']['type_multiple_error'] = isset($messages[6]) ? I18n::translate($messages[6]) : 'type_multiple-msg';
        }

        if (!isset($configuration['messages']['zip-type_error'])) {
            $configuration['messages']['zip-type_error'] = isset($messages[7]) ? I18n::translate($messages[7]) : 'zip-type_error-msg';
        }

        if (!isset($configuration['messages']['type_zip_error'])) {
            $configuration['messages']['type_zip_error'] = isset($messages[8]) ? I18n::translate($messages[8]) : 'type_zip_error-msg';
        }

        if (!isset($configuration['messages']['extension_zip_type_error'])) {
            $configuration['messages']['extension_zip_type_error'] = isset($messages[9]) ? I18n::translate($messages[9]) : 'extension_zip_type_error-msg {0}';
        }

        if (!isset($configuration['messages']['delete_file'])) {
            $configuration['messages']['delete_file'] = isset($messages[4]) ? I18n::translate($messages[4]) : 'delete-msg';
        }
            ...
            if (isset($FILE)) {
                if (!@move_uploaded_file($FILE['tmp_name'], $FILE['tmp_yform_name'])) {
                    if (!@copy($FILE['tmp_name'], $FILE['tmp_yform_name'])) {
                        ...
                        @chmod($FILE['tmp_yform_name'], 0o666 & ~umask());
                    }
                }
            }
        ...
        if (Core::isBackend()
            && (
                Request::request('rex_upload_downloadfile', 'string') == $this->getName()
            )
                && '' != $filename
                && '' != $filepath
        ) {
        ...
        if ($this->needsOutput() && $this->isViewable()) {
            if (!$this->isEditable()) {
                $this->params['form_output'][$this->getId()] = $this->parse(['value.upload-view.tpl.php', 'view'], [
                ...
                $this->params['form_output'][$this->getId()] = $this->parse('upload', [
                    'unique' => $this->upload_getSessionKey(),
                    'filename' => $filename,
                    'error_messages' => $configuration['messages'],
                    'download_link' => $download_link,
            ...
            if (file_exists($FILE['tmp_yform_name'])) {
                ...
                if (!move_uploaded_file($FILE['tmp_yform_name'], $upload_filefolder)) {
                    if (!copy($FILE['tmp_yform_name'], $upload_filefolder)) {
                        ...
                        @chmod($upload_filefolder, 0o666 & ~umask());
    ...
    public function upload_getFolder(): string
    {
        ...
        return Path::addonData('yform', 'upload/' . implode('/', $folders));
    }
    ...
    public function getDefinitions(): array
    {
        return [
            ...
            'values' => [
                ...
                'sizes' => ['type' => 'text',    'label' => I18n::msg('yform_values_upload_sizes')],
                'types' => ['type' => 'text',    'label' => I18n::msg('yform_values_upload_types'),    'notice' => I18n::msg('yform_values_upload_types_notice'), 'default' => ''],
                'required' => ['type' => 'boolean', 'label' => I18n::msg('yform_values_upload_required')],
                'messages' => ['type' => 'text',    'label' => I18n::msg('yform_values_upload_messages'), 'notice' => I18n::msg('yform_values_upload_messages_notice')],
                'notice' => ['type' => 'text',    'label' => I18n::msg('yform_values_defaults_notice')],
                'config' => ['type' => 'textarea', 'attributes' => ['class' => 'form-control codemirror', 'data-codemirror-mode' => 'application/json'], 'label' => I18n::msg('yform_values_upload_config'), 'notice' => I18n::msg('yform_values_upload_config_notice')],
            ],
            'description' => I18n::msg('yform_values_upload_description'),
            'db_type' => ['text'],
            'multi_edit' => true,
        ];
    }
    ...
    public static function upload_getDownloadLink(string $table_name, string $field_name, int $data_id): string
    {
        if ('' != $table_name && '' != $field_name && 0 < $data_id) {
            return Url::backendPage('yform/manager/data_edit', [
    ...
    public static function getListValue($params)
    {
        ...
        $return = $value;
        if (Core::isBackend() && $list) {
            $field = new Field($params['params']['field']);
            if ('' != $value) {
                $return = '<a href="' . self::upload_getDownloadLink($field->getElement('table_name'), $field->getElement('name'), (int) $list->getValue('id')) . '" title="' . escape($title) . '">' . escape($value) . '</a>';
            }
        }
    ...
    {
        $sessionVars = Request::session($this->upload_getSessionKey(), 'array', []);
        $sessionVars[$key] = $value;
        Request::setSession($this->upload_getSessionKey(), $sessionVars);
    }
    ...
    public function upload_getSessionVar(string $key, string $varType = 'string', mixed $default = ''): mixed
    {
        ...
        if (array_key_exists($key, $sessionVars)) {
            return Type::cast($sessionVars[$key], $varType);
        }
    ...
    {
        Request::setSession($this->upload_getSessionKey(), []);
    }
}
