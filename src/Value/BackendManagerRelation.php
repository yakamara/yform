...
#[AsValue('be_manager_relation')]
class BackendManagerRelation extends AbstractValue
{
    ...
    public function enterObject()
    {
        ...
        if (0 == $this->relation['relation_type'] || 1 == $this->relation['relation_type']) {
            ...
            if (!$this->isEditable()) {
                $this->params['form_output'][$this->getId()] = $this->parse(['value.be_manager_relation-view.tpl.php', 'view'], ['options' => $viewOptions]);
            } else {
                $this->params['form_output'][$this->getId()] = $this->parse('be_manager_relation', compact('options'));
            }
        }
        ...
        if (2 == $this->relation['relation_type'] || 3 == $this->relation['relation_type']) {
            ...
            if (!$this->isEditable()) {
                ...
                $this->params['form_output'][$this->getId()] = $this->parse('be_manager_relation', compact('valueName', 'options', 'link'));
            }
        }
        ...
        if (4 == $this->relation['relation_type']) {
            ...
            $this->params['form_output'][$this->getId()] = $this->parse('be_manager_relation', compact('valueName', 'options', 'link', 'send'));
        }

        // ------------------------------------ INLINE, 1-n
        if (5 == $this->relation['relation_type']) {
            $warning = false;
            $table = Table::get($this->relation['target_table']);
            ...
            $relations = Dataset::query($this->relation['target_table'])
            ...
            if (!$send) {
                foreach ($relations as $counter => $relation) {
                    ...
                    $yform->canEdit(Authorization::onAttribute('EDIT', $table, Core::getUser()));
                    $yform->canView(Authorization::onAttribute('VIEW', $table, Core::getUser()));
            ...
            if ($this->isViewable()) {
                if (!$this->isEditable()) {
                    $this->params['form_output'][$this->getId()] = $this->parse('be_manager_inline_relation.view', compact('forms', 'fieldkey', 'prioFieldName', 'relationKey'));
                } else {
                    $this->params['form_output'][$this->getId()] = $this->parse('be_manager_inline_relation', compact('forms', 'prototypeForm', 'fieldkey', 'prioFieldName', 'relationKey'));
                }
            }
        }
    }
    ...
    public function postSave(): void
    {
        if ($this->needsOutput()) {
            if (5 == $this->relation['relation_type'] && $this->params['main_id'] > 0) {
                ...
                $currentRelationsQuery = Dataset::query($this->relation['target_table'])
                    ->where($this->relation['target_field'], $this->params['main_id'])
                    ->find();
    ...
    public function getDefinitions(): array
    {
        return [
            ...
            'values' => [
                ...
                'table' => ['type' => 'table',   'label' => I18n::msg('yform_values_be_manager_relation_table')],
                'field' => ['type' => 'text',    'label' => I18n::msg('yform_values_be_manager_relation_field')],
                'type' => ['type' => 'choice',  'label' => I18n::msg('yform_values_be_manager_relation_type'), 'choices' => [
                    '0' => 'Single (select)',
                    '2' => 'Single (popup)',
                    '1' => 'Multiple (select)',
                    '3' => 'Multiple (popup)',
                ...
                'empty_option' => ['type' => 'boolean', 'label' => I18n::msg('yform_values_be_manager_relation_empty_option')],
                'empty_value' => ['type' => 'text',    'label' => I18n::msg('yform_values_be_manager_relation_empty_value')],
                'size' => ['type' => 'text', 'name' => 'boxheight',    'label' => I18n::msg('yform_values_be_manager_relation_size')],
                'filter' => ['type' => 'textarea', 'label' => I18n::msg('yform_values_be_manager_relation_filter')],
                'relation_table' => ['type' => 'table', 'label' => I18n::msg('yform_values_be_manager_relation_relation_table'), 'empty_option' => 1],
            ...
            'description' => I18n::msg('yform_values_be_manager_relation_description'),
            'db_type' => ['text', 'varchar(191)', 'int', 'int(10) unsigned'],
            'formbuilder' => false,
            'hooks' => [
                'preCreate' => static function (Field $field) {
                    return !$field->getElement('relation_table') && ('4' != $field->getElement('type') && '5' != $field->getElement('type'));
                },
            ],
            'multi_edit' => static function (Field $field) {
    ...
    public static function getListValue($params)
    {
        ...
        switch ($field['type']) {
            ...
            case 5:
                ...
                return '<a href="' . $link . '">' . I18n::translate($field['label']) . '</a>';
            case 0:
            case 1:
            case 2:
            case 3:
