...
#[AsValue('google_geocode')]
class GoogleGeoCode extends AbstractValue
{
    public function enterObject()
    {
        // Casts: the column is nullable and `default` is unset unless configured.
        $values = explode(',', (string) $this->getValue());
        $default = explode(',', (string) $this->getElement('default'));
        ...
        if ($this->needsOutput() && $this->isViewable()) {
            if (!$this->isEditable()) {
                $this->params['form_output'][$this->getId()] = $this->parse(['value.google_geocode-view.tpl.php', 'view']);
            } else {
                $this->params['form_output'][$this->getId()] = $this->parse('text');
                $this->params['form_output'][$this->getId()] .= $this->parse(
                    'google_geocode', compact('value', 'mapWidth', 'mapHeight', 'mapZoom', 'address', 'googleapikey'),
                );
            }
        }
