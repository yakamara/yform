...
abstract class AbstractValue extends AbstractBase
{
    ...
    /**
     * Renders one of yform's value fragments.
     *
     * `$template` is a fragment key, not a path: "text", or a list of candidates to
     * fall back through (["be_link.view", "view"]). The backend or frontend variant
     * is chosen from the current context.
     *
     * The fragment gets this value object as `$yfield` plus everything in `$params` —
     * inside a fragment `$this` is the Fragment, not the value object.
     *
    ...
    {
        return Fragment::render($template, 'value', ['yfield' => $this] + $params);
    }
    ...
    public function getDatabaseFieldNull()
    {
        ...
        // Nullable unless the field type explicitly demands NOT NULL. See the note in fixups for why
        // this differs from yform 5, where the default was NOT NULL.
        return $definitions['db_null'] ?? true;
    }
