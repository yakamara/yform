...
class Tools
{
    public static function listFormat($p, $value = '')
    {
        ...
        if ('label' == $p['field']) {
            $p['value'] = I18n::translate((string) $p['value']);
        }
    ...
    {
        return self::listFormat($p, $p['list']->getColumnLink(I18n::msg('yform_function'), '<i class="rex-icon rex-icon-editmode"></i> ' . I18n::msg('yform_edit')));
    }

    public static function deleteFormat($p)
    {
        return self::listFormat($p, $p['list']->getColumnLink(I18n::msg('yform_delete'), '<i class="rex-icon rex-icon-delete"></i> ' . I18n::msg('yform_delete')));
    }
}
