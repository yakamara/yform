...
namespace Yakamara\YForm\ListView;
...
// This list is a fork of core's View\DataList and shares its column-option keys, which core declares as
// plain constants at file scope in DataList.php — unguarded, so defining them here first would make
// *core* warn about the redefinition. Loading DataList is what declares them, and leaves core as their
// single owner.
class_exists(DataList::class);
 ...
 */
class YList implements UrlProviderInterface
{
    use FactoryTrait;
    ...
    protected function __construct(Query $query, int $rowsPerPage = 30, $listName = null, $debug = false)
    {
        ...
        if ('' != $sortColumn) {
            ...
            $this->query->orderBy($sortColumn, 'asc' == $sortType ? 'asc' : 'desc');
        }
        ...
        $this->query->limit($startRow, $rowsPerPage);

        $this->columnNames[] = 'id';
        foreach ($this->query->getTable()->getFields() as $field) {
            if ('value' == $field->getType() && 'none' != $field->getDatabaseFieldType()) {
                $this->columnNames[] = $field->getName();
            }
        }
    ...
    /**
     * REDAXO 6 dropped the $escape argument from both UrlProviderInterface::getUrl() and
     * Url::backendController(); query values are urlencoded by Str::buildQuery, and escaping for HTML
     * output is the caller's job. Core's own DataList emits the result into href="…" unescaped too.
     */
    #[Override]
    ...
    {
        return str_replace(['###' . $varname . '###', '___' . $varname . '___'], (string) escape((string) $this->getValue($varname)), $string);
    }
    ...
    public function get()
    {
        Extension::dispatch(new ExtensionPoint('YFORM_LIST_GET', $this, [], true));
        ...
        if ($nbRows > 0) {
            ...
            foreach ($this->query->find() as $item) {
