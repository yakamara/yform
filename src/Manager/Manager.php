...
use Yakamara\YForm\ListView\Tools;
...
class Manager
{
    ...
    {
        Extension::dispatch(new ExtensionPoint('YFORM_MANAGER_DATA_PAGE', $this));

        $func = Request::request('func', 'string', '');
        $data_id = Request::request('data_id', 'int', 0);
        ...
        $rex_yform_filter = Request::request('rex_yform_filter', 'array');
        ...
        $rex_yform_set = Request::request('rex_yform_set', 'array');
        ...
        $rex_yform_list = [];
        $rex_yform_list['list'] = Request::request('list', 'string');
        $rex_yform_list['sort'] = Request::request('sort', 'string');
        $rex_yform_list['sorttype'] = Request::request('sorttype', 'string');
        $rex_yform_list['start'] = Request::request($rex_yform_list['list'] . '_start', 'int', null) ?? Request::request('start', 'int', null) ?? 0;

        $_csrf_key = $this->table->getCSRFKey();
        $rex_yform_list += CsrfToken::factory($_csrf_key)->getUrlParams();

        $popup = false;
        $rex_yform_manager_opener = Request::request('rex_yform_manager_opener', 'array');
        if (isset($rex_yform_manager_opener['id']) && '' != $rex_yform_manager_opener['id']) {
            $popup = true; // id, field, multiple
        }

        $rex_yform_manager_popup = Request::request('rex_yform_manager_popup', 'int');
        if (1 == $rex_yform_manager_popup) {
            $popup = true;
        }

        $rex_yform_filter = Extension::dispatch(new ExtensionPoint('YFORM_MANAGER_DATA_EDIT_FILTER', $rex_yform_filter, [
            'table' => $this->table,
        ]));

        $rex_yform_set = Extension::dispatch(new ExtensionPoint('YFORM_MANAGER_DATA_EDIT_SET', $rex_yform_set, [
            'table' => $this->table,
        ]));

        $searchObject = new Search($this->table);
        $searchObject
        ...
        $description = $popup || ('' == $this->table->getDescription()) ? '' : '<p>' . nl2br(escape(I18n::translate($this->table->getDescription()))) . '</p>';
        ...
        echo Extension::dispatch(
            new ExtensionPoint(
                'YFORM_MANAGER_DATA_PAGE_HEADER',
                View::title(I18n::msg('yform_table') . ': ' . escape($this->table->getNameLocalized()) . ' <small>[' . escape($this->table->getTablename()) . ']</small>', '') . $description,
                [
                    'yform' => $this,
                ],
            ),
        ...
        echo Extension::dispatch(new ExtensionPoint('YFORM_MANAGER_REX_INFO', ''));
        ...
        if ($this->table->isGranted('EDIT', Core::getUser())) {
        ...
        if ('' != $func) {
            if (!CsrfToken::factory($_csrf_key)->isValid()) {
                $mainMessages[] = [
                    'type' => 'error',
                    'message' => I18n::msg('csrf_token_invalid'),
                ];
            }
        }
        ...
        switch ($func) {
            case 'import':
                if (!$popup && $this->hasDataPageFunction('import')) {
                    $mainMessages[] = [
                        'type' => 'info',
                        'message' => '<b>&laquo; ' . I18n::msg('yform_back_to_overview') . '</b>',
                        'link' => 'index.php?' . http_build_query(array_merge($rex_link_vars)),
                    ];

                    ob_start();
                    include Path::addon('yform', 'pages/manager.data_import.php');
            ...
            case 'history':
                if (!$popup) {
                    ...
                    ob_start();
                    include Path::addon('yform', 'pages/manager.data_history.php');
            ...
            case 'delete':
                if ($data_id && $this->hasDataPageFunction('delete')) {
                    if ($this->table->getRawDataset($data_id)->delete()) {
                        $mainMessages[] = [
                            'type' => 'success',
                            'message' => I18n::msg('yform_datadeleted'),
                        ];
                    }
                }
                break;
            case 'dataset_delete':
                if ($this->hasDataPageFunction('truncate_table')) {
                    ...
                    $mainMessages[] = [
                        'type' => 'success',
                        'message' => I18n::msg('yform_dataset_deleted'),
                    ];
                }
                break;
            case 'truncate_table':
                if (!$popup && $this->hasDataPageFunction('truncate_table')) {
                    ...
                    $mainMessages[] = [
                        'type' => 'success',
                        'message' => I18n::msg('yform_table_truncated'),
                    ];
                }
                break;
            case 'dataset_export':
            ...
            case 'collection_edit':
                if (
                    ...
                    $yform->canEdit(Authorization::onAttribute('EDIT', $this->table, Core::getUser()));
                    $yform->canView(Authorization::onAttribute('VIEW', $this->table, Core::getUser()));
                    ...
                    if ('' != Request::request('rex_yform_show_formularblock', 'string')) {
                        ...
                        foreach ($this->table->getFields() as $field) {
                            $class = FieldRegistry::getClass($field->getType(), $field->getTypeName());
                    ...
                    switch ($func) {
                        ...
                        case 'edit':
                            ...
                            $buttonLabels = I18n::msg('yform_save') . ',' . I18n::msg('yform_save_apply');
                            break;
                        case 'add':
                            $buttonLabels = I18n::msg('yform_add') . ',' . I18n::msg('yform_add_apply');
                            break;
                        case 'collection_edit':
                            $buttonLabels = I18n::msg('yform_save') . ',' . I18n::msg('yform_save_apply');
                    }
                    ...
                    if ($yform->isEditable()) {
                        ...
                        if (isset($rex_yform_manager_opener['id'])) {
                            ...
                            /*return '<a href="javascript:setYFormDataset('.$params['params']['opener_id'].',###id###,\''.escape(
                                    $value,
                                    'js'
                                ).' [id=###id###]\','.$params['params']['opener_multiple'].')">'.I18n::msg(
                                    'yform_data_select'
                                ).'</a>';
                            */
                        }
                    ...
                    try {
                        ...
                        $sql_db->transactional(static function () use (&$form, &$yform, $data, $func) {
                            ...
                            if ('clone' == $func) {
                                $afterFieldsExecuted = static function (YForm $yform) use ($afterFieldsExecuted) {
                                    ...
                                    foreach ($yform->objparams['values'] as $k => $v) {
                                        ...
                                        if ($v instanceof Submit) {
                                            $yform->objparams['form_output'][$k] = str_replace(
                                                [I18n::msg('yform_save') . '</button', I18n::msg('yform_save_apply') . '</button'],
                                                [I18n::msg('yform_add') . '</button', I18n::msg('yform_add_apply') . '</button'],
                        ...
                        if ($yform->objparams['form_show'] || $yform->objparams['form_showformafterupdate']) {
                            if ('collection_edit' === $func) {
                                $title = I18n::msg('yform_editdata_collection', $data->count());
                            } elseif ('add' == $func) {
                                $title = I18n::msg('yform_adddata');
                            } elseif ('clone' == $func) {
                                $title = I18n::msg('yform_clonedata', $data_id);
                            } else {
                                $title = I18n::rawMsg('yform_editdata', $data_id);
                            }
                            ...
                            $mainMessages[] = [
                                'type' => 'info',
                                'message' => I18n::msg('yform_back_to_overview'),
                                'link' => 'index.php?' . http_build_query(array_merge($rex_link_vars)),
                            ];
                        }
                        ...
                        if ($yform->objparams['actions_executed']) {
                            if ($yform->hasWarnings()) {
                                $mainMessages[] = [
                                    'type' => 'error',
                                    'message' => I18n::msg('yform_errors_occurred'),
                                ];
                            } elseif (!$yform->isEditable()) {
                            } elseif ('collection_edit' == $func) {
                                $mainMessages[] = [
                                    'type' => 'info',
                                    'message' => I18n::msg('yform_thankyouforupdates'),
                                ];
                            } elseif ('edit' == $func) {
                                $mainMessages[] = [
                                    'type' => 'info',
                                    'message' => I18n::msg('yform_thankyouforupdate'),
                                ];
                            } else {
                                // -> add
                                $mainMessages[] = [
                                    'type' => 'info',
                                    'message' => I18n::msg('yform_thankyouforentry'),
                                ];
                            }
                        }
                    } catch (Throwable $e) {
                        $mainMessages[] = [
                            'type' => 'error',
                            'message' => I18n::msg('yform_editdata_collection_error_abort', $e->getMessage()),
                        ];
                    }
                }
                break;
        ...
        if (!$popup) {
            ...
            if ($this->table->isGranted('EDIT', Core::getUser())) {
                if ($this->table->isMassEditAllowed()) {
                    $item = [];
                    $item['label'] = I18n::msg('yform_edit');
                    $item['url'] = 'index.php?' . http_build_query(array_merge(['func' => 'collection_edit'], $rex_link_vars));
                    $item['attributes']['class'][] = 'btn-default';
                    $item['attributes']['data-confirm'][] = I18n::msg('yform_dataset_edit_confirm');
                    $dataset_links[] = $item;
                }

                if (Core::getUser()->admin || (1 == $this->table->isExportable() && $this->hasDataPageFunction('export'))) {
                    $item = [];
                    $item['label'] = I18n::msg('yform_export');
                ...
                if ($this->table->isMassDeletionAllowed() && $this->hasDataPageFunction('truncate_table')) {
                    $item = [];
                    $item['label'] = I18n::msg('yform_delete');
                    ...
                    $item['attributes']['onclick'][] = 'return confirm(\'' . I18n::msg('yform_dataset_delete_confirm') . '\');';
                    $dataset_links[] = $item;
                }
            }
            ...
            if ($this->table->isGranted('EDIT', Core::getUser())) {
                if (Core::getUser()->admin || ($this->table->isImportable() && $this->hasDataPageFunction('import'))) {
                    $item = [];
                    $item['label'] = I18n::msg('yform_import');
                ...
                if ($this->table->isMassDeletionAllowed() && $this->hasDataPageFunction('truncate_table')) {
                    $item = [];
                    $item['label'] = I18n::msg('yform_truncate_table');
                    $item['url'] = 'index.php?' . http_build_query(array_merge(['func' => 'truncate_table'], $rex_link_vars));
                    $item['attributes']['class'][] = 'btn-delete';
                    $item['attributes']['id'] = 'truncate-table';
                    $item['attributes']['onclick'][] = 'return confirm(\'' . I18n::msg('yform_truncate_table_confirm') . '\');';
                    $table_links[] = $item;
                }
                if ($this->table->hasHistory()) {
                    $item = [];
                    $item['label'] = I18n::msg('yform_history');
            ...
            $field_links = [];
            if ($this->table->isGranted('EDIT', Core::getUser()) && Core::getUser()->admin) {
                $item = [];
                $item['label'] = I18n::msg('yform_edit');
            ...
            ['dataset_links' => $dataset_links, 'table_links' => $table_links, 'field_links' => $field_links] = Extension::dispatch(
                new ExtensionPoint(
            ...
            if (count($dataset_links) > 0) {
                ...
                $panel_options[] = '<small class="rex-panel-option-title">' . I18n::msg('yform_dataset') . '</small> ' . $fragment->parse('core/buttons/button_group.php');
            }
            ...
            if (count($table_links) > 0) {
                ...
                $panel_options[] = '<small class="rex-panel-option-title">' . I18n::msg('yform_table') . '</small> ' . $fragment->parse('core/buttons/button_group.php');
            }
        ...
        if ($this->table->isGranted('EDIT', Core::getUser())) {
            $actionButtons['edit'] = [
                ...
                'content' => '<i class="rex-icon rex-icon-editmode"></i> ' . I18n::msg('yform_edit'),
            ];

            $actionButtons['clone'] = [
                'params' => array_merge($actionButtonParams, ['func' => 'clone']),
                'content' => '<i class="rex-icon rex-icon-add"></i> ' . I18n::msg('yform_clone'),
            ];

            if ($this->hasDataPageFunction('delete')) {
                $actionButtons['delete'] = [
                    'params' => array_merge($actionButtonParams, ['func' => 'delete']),
                    'content' => '<i class="rex-icon rex-icon-delete"></i> ' . I18n::msg('yform_delete'),
                    'attributes' => [
                        'onclick' => 'return confirm(\' id=___id___ ' . I18n::msg('yform_delete') . ' ?\')',
                    ],
                ];
            }
            ...
            if (!$popup && $this->table->hasHistory()) {
                $actionButtons['history'] = [
                    ...
                    'content' => '<i class="rex-icon fa-history"></i> ' . I18n::msg('yform_history'),
                ];
            }
        } else {
            $actionButtons['view'] = [
                'params' => array_merge($actionButtonParams, ['func' => 'edit']),
                'content' => '<i class="rex-icon rex-icon-view"></i> ' . I18n::msg('yform_view'),
            ];
        }

        $actionButtons = Extension::dispatch(
            new ExtensionPoint(
        ...
        $filterMessages = [];
        if ($rex_yform_filter) {
            $getFilter = static function (Field $field, $value, $table) {
                ...
                return '<b>' . I18n::translate($field->getLabel()) . ':</b> ' . $listValues;
            };
    ...
    public function getDataListQuery(Query $query, array $rex_filter = [], ?Search $searchObject = null)
    {
            ...
            if (array_key_exists($field->getName(), $rex_filter) && 'value' == $field->getType()) { //  && $field->isSearchable()
                ...
                if (null !== $class && method_exists($class, 'getSearchFilter')) {
                    ...
                    if (!$query instanceof Query) {
                        throw new Exception('getSearchFilter in ' . $class . ' does not return a Query');
                    }
                }
            }
        }
        ...
        return Extension::dispatch(new ExtensionPoint('YFORM_DATA_LIST_QUERY', $query, ['filter' => $rex_filter]));
    }
    ...
    public function getFieldPage()
    {
        ...
        $func = Request::request('func', 'string', 'list');
        $type_id = Request::request('type_id', 'string');
        $type_name = Request::request('type_name', 'string');
        $field_id = Request::request('field_id', 'int');
        ...
        $TYPE = ['value' => I18n::msg('yform_values'), 'validate' => I18n::msg('yform_validates'), 'action' => I18n::msg('yform_action')];

        $table = $this->table;

        $table_info = '<b>' . escape($table->getNameLocalized()) . ' [<a href="index.php?page=yform/manager/table_edit&start=0&table_id=' . $table->getId() . '&func=edit">' . $table->getTableName() . '</a>]</b> ';
        echo Message::info($table_info);
        ...
        $type_real_field = Request::request('type_real_field', 'string');
        ...
        if ('choosenadd' == $func) {
            ...
            if (!$table->hasId()) {
                $content[] = I18n::msg('yform_id_is_missing') . '' . I18n::msg('yform_id_missing_info');
            } else {
            ...
            $fragment = new Fragment();
            $fragment->setVar('title', I18n::msg('yform_choosenadd'));
            $fragment->setVar('body', I18n::msg('yform_choosenadd_description'), false);
            echo $fragment->parse('core/page/section.php');
            ...
            $table_echo = '<a class="btn btn-default" href="index.php?' . $link_vars . '&amp;table_name=' . $table->getTableName() . '">' . I18n::msg('yform_back_to_overview') . '</a>';
        ...
        if (('add' == $func || 'edit' == $func) && isset($types[$type_id][$type_name])) {
            $field = new Field(['type_id' => $type_id, 'type_name' => $type_name]);
            ...
            $yform->setValueField('prio', ['prio', I18n::msg('yform_values_defaults_prio'), ['type_id', 'type_name', 'name', 'label'], ['table_name']]);
            ...
            foreach ($types[$type_id][$type_name]['values'] as $k => $v) {
                ...
                switch ($v['type']) {
                    case 'name':
                        $v['notice'] = ($v['notice'] ?? '');
                        if ('edit' == $func) {
                            $yform->setValueField('showvalue', [$k_field, I18n::msg('yform_values_defaults_name'), 'notice' => $v['notice']]);
                        } else {
                            ...
                            $yform->setValueField('text', [$k_field, I18n::msg('yform_values_defaults_name'), $v['value'], 'notice' => $v['notice']]);
                            $yform->setValidateField('empty', [$k_field, I18n::msg('yform_validatenamenotempty')]);
                            $yform->setValidateField('preg_match', [$k_field, '/(([a-zA-Z])+([a-zA-Z0-9\\_])*)/', I18n::msg('yform_validatenamepregmatch')]);
                            $yform->setValidateField('customfunction', [$k_field, self::class . '::checkField', ['table_name' => $table->getTableName()], I18n::msg('yform_validatenamecheck')]);
                        }
                        break;
                    ...
                    case 'no_db':
                        ...
                        $yform->setValueField('checkbox', ['name' => $k_field, 'label' => I18n::msg('yform_donotsaveindb'), 'values' => 'no_db', 'default' => $v['default']]);
                        break;
                    ...
                    case 'table':
                        $_tables = Table::getAll();
                        $_options = [];
                        if (isset($v['empty_option']) && $v['empty_option']) {
                            $_options[0] = '–=';
                        }
                        foreach ($_tables as $_table) {
                            $_options[$_table['table_name']] = str_replace('=', '-', I18n::translate($_table['name']) . ' [' . $_table['table_name'] . ']') . '=' . $_table['table_name'];
            ...
            $yform->setActionField('showtext', ['', '<p>' . I18n::msg('yform_thankyouforentry') . '</p>']);
            $yform->setObjectparams('main_table', Field::table());

            switch ($func) {
                case 'edit':
                    $yform->setObjectparams('submit_btn_label', I18n::msg('yform_field_update'));
                    $yform->setHiddenField('field_id', $field_id);
                    $yform->setActionField('manage_db', [Field::table(), "id=$field_id"]);
                    $yform->setObjectparams('main_id', $field_id);
                    $yform->setObjectparams('main_where', "id=$field_id");
                    $sql = Sql::factory();
                    $sql->setQuery('SELECT * FROM ' . Field::table() . " WHERE id=$field_id");
                ...
                case 'add':
                default:
                    $yform->setObjectparams('submit_btn_label', I18n::msg('yform_field_add'));
                    $yform->setActionField('manage_db', [Field::table()]);
                    break;
            }
            ...
            if ('value' == $type_id) {
                ...
                $yform->setValueField('choice', ['name' => 'db_type', 'label' => I18n::msg('yform_field_db_type'), 'choices' => $db_choices, 'default' => $default]);

                if (!$field->isHiddenInListDisabled()) {
                    $yform->setValueField('checkbox', ['name' => 'list_hidden', 'label' => I18n::msg('yform_hideinlist'), 'default' => '1']);
                }

                if (!$field->isSearchableDisabled()) {
                    $yform->setValueField('checkbox', ['name' => 'search', 'label' => I18n::msg('yform_useassearchfieldalidatenamenotempty'), 'default' => '0']);
                }
            ...
            if ($yform->objparams['form_show']) {
                if ('add' == $func) {
                    $title = I18n::msg('yform_addfield') . ' "' . $type_name . '"';
                } else {
                    $title = I18n::msg('yform_editfield') . ' "' . $type_name . '"';
                }
                ...
                switch ($func) {
                    case 'edit':
                        ...
                        echo Message::success(I18n::msg('yform_thankyouforupdate'));
                        break;
                    case 'add':
                    default:
                        Api::generateTableAndFields($table);
                        echo Message::success(I18n::msg('yform_thankyouforentry'));
                        break;
                }
                $func = 'list';
            }
        ...
        if ('delete' == $func) {
            if (!CsrfToken::factory($_csrf_key)->isValid()) {
                ...
                $sf->setQuery('select * from ' . Field::table() . ' where table_name="' . $table->getTableName() . '" and id=' . $field_id);
                $sfa = $sf->getArray();
                if (1 == count($sfa)) {
                    $query = 'delete from ' . Field::table() . ' where table_name="' . $table->getTableName() . '" and id=' . $field_id;
                    $delsql = Sql::factory();
                    $delsql->setDebug(self::$debug);
                    $delsql->setQuery($query);
                    echo Message::success(I18n::msg('yform_tablefielddeleted'));
                    Api::generateTableAndFields($table);
                } else {
                    echo Message::warning(I18n::msg('yform_tablefieldnotfound'));
                }
                $func = 'list';
            }
        }
        ...
        if ('updatetable' == $func) {
            Api::generateTableAndFields($table);
            echo Message::info(I18n::msg('yform_tablesupdated'));
            $func = 'list';
        }

        if ('updatetablewithdelete' == $func) {
            Api::generateTableAndFields($table, true);
            echo Message::info(I18n::msg('yform_tablesupdated'));
            $func = 'list';
        }
        ...
        if ('show_form_notation' == $func) {
            ...
            $notation_php_pre = [
                ...
                '$yform->setObjectparams(\'form_action\',Url::article((int) \'REX_ARTICLE_ID\'));',
            ...
            foreach ($formbuilder_fields as $field) {
                ...
                foreach ($definitions['values'] as $key => $_) {
                    ...
                    if (isset($field[$key])) {
                        $values[] = escape($field[$key]);
                    } elseif (isset($field['f' . $i])) {
                        $values[] = escape($field['f' . $i]);
                    } else {
                        $values[] = '';
                    }
                    ++$i;
                ...
                if ('value' == $field['type_id']) {
                    ...
                    $notation_email .= "\n" . I18n::translate($field['label']) . ': REX_YFORM_DATA[field="' . $field['name'] . '"]';
        ...
        if ('list' == $func) {
            ...
            if ($show_list) {
                ...
                $fragment->setVar('buttons', [
                    ...
                    [
                        'label' => I18n::msg('yform_manager_show_form_notation'),
                    ...
                    [
                        'label' => I18n::msg('yform_updatetable'),
                        'url' => $context->getUrl(['table_name' => $table->getTableName(), 'func' => 'updatetable'] + CsrfToken::factory($_csrf_key)->getUrlParams()),
                        'attributes' => [
                            'class' => [
                                'btn-default',
                            ],
                    ...
                    [
                        'label' => I18n::msg('yform_updatetable') . ' ' . I18n::msg('yform_updatetable_with_delete'),
                        'url' => $context->getUrl(['table_name' => $table->getTableName(), 'func' => 'updatetablewithdelete'] + CsrfToken::factory($_csrf_key)->getUrlParams()),
                        'attributes' => [
                            'class' => [
                                'btn-default',
                            ],
                            'onclick' => [
                                'return confirm(\'' . I18n::msg('yform_updatetable_with_delete_confirm') . '\')',
                            ],
                        ], 0,
                    ],
                ], false);
                ...
                $sql = 'select id, prio, type_id, type_name, name, label from ' . Field::table() . ' where table_name="' . $table->getTableName() . '"';
                $list = DataList::factory($sql, rowsPerPage: 500, defaultSort: [
                    'prio' => 'asc',
                ]);
                ...
                $thIcon = '<a class="rex-link-expanded" href="' . $list->getUrl(['table_name' => $table->getTableName(), 'func' => 'choosenadd']) . '" ' . Core::getAccesskey(I18n::msg('add'), 'add') . '><i class="rex-icon rex-icon-add"></i></a>';
                ...
                // $list->setColumnLayout('prio', ['<th class="rex-table-priority">###VALUE###</th>', '<td class="rex-table-priority" data-title="' . I18n::msg('yform_manager_table_prio_short') . '">###VALUE###</td>']);
                $list->setColumnLayout('prio', ['<th>###VALUE###</th>', '###VALUE###']);
                $list->setColumnFormat('prio', 'custom', Tools::class . '::listFormat');

                $list->setColumnLabel('type_id', I18n::msg('yform_manager_type_id'));
                $list->setColumnLayout('type_id', ['<th>###VALUE###</th>', '###VALUE###']);
                $list->setColumnFormat('type_id', 'custom', Tools::class . '::listFormat');

                $list->setColumnLabel('type_name', I18n::msg('yform_manager_type_name'));
                $list->setColumnLayout('type_name', ['<th>###VALUE###</th>', '###VALUE###']);
                $list->setColumnFormat('type_name', 'custom', Tools::class . '::listFormat');

                $list->setColumnLabel('name', I18n::msg('yform_values_defaults_name'));
                $list->setColumnLayout('name', ['<th>###VALUE###</th>', '###VALUE###']); // ###VALUE###
                $list->setColumnFormat('name', 'custom', Tools::class . '::listFormat');

                $list->setColumnLabel('label', I18n::msg('yform_values_defaults_label'));
                $list->setColumnLayout('label', ['<th>###VALUE###</th>', '###VALUE###']); // ###VALUE###
                $list->setColumnFormat('label', 'custom', Tools::class . '::listFormat');

                $list->addColumn(I18n::msg('yform_function'), '<i class="rex-icon rex-icon-editmode"></i> ' . I18n::msg('yform_edit'));
                $list->setColumnParams(I18n::msg('yform_function'), ['field_id' => '###id###', 'func' => 'edit', 'type_name' => '###type_name###', 'type_id' => '###type_id###']);
                $list->setColumnLayout(I18n::msg('yform_function'), ['<th class="rex-table-action" colspan="2">###VALUE###</th>', '###VALUE###']);
                $list->setColumnFormat(I18n::msg('yform_function'), 'custom', Tools::class . '::editFormat');

                $list->addColumn(I18n::msg('yform_delete'), '<i class="rex-icon rex-icon-delete"></i> ' . I18n::msg('yform_delete'));
                $list->setColumnParams(I18n::msg('yform_delete'), ['field_id' => '###id###', 'func' => 'delete'] + CsrfToken::factory($_csrf_key)->getUrlParams());
                $list->setColumnLayout(I18n::msg('yform_delete'), ['', '###VALUE###']);
                $list->setColumnFormat(I18n::msg('yform_delete'), 'custom', Tools::class . '::deleteFormat');
                $list->addLinkAttribute(I18n::msg('yform_delete'), 'onclick', 'return confirm(\' [###type_id###, ###type_name###, ###name###] ' . I18n::msg('yform_delete') . ' ?\')');

                $content = $list->get();

                $fragment = new Fragment();
                $fragment->setVar('title', I18n::msg('yform_manager_tablefield_overview'));
    ...
    {
        $q = 'select * from ' . Field::table() . ' where table_name="' . $p['table_name'] . '" and type_id="value" and ' . $l . '="' . $v . '" LIMIT 1';
        $c = Sql::factory();
    ...
    public function createTable($mifix, $data_table, $params = [], $debug = false)
    {
        ...
        $c = Sql::factory();
        $c->setDebug($debug);
        $c->setQuery('DELETE FROM ' . Table::table() . ' where table_name="' . $data_table . '"');
        $c->setTable(Table::table());
    ...
    public static function url(string $tableName, int $id = 0, array $params = []): string
    {
        ...
        if ($id > 0) {
            ...
            $params += CsrfToken::factory($_csrf_key)->getUrlParams();
        }

        return Url::backendPage('yform/manager/data_edit', $params);
    }
}
