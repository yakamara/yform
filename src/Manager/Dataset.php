...
class Dataset
{
    ...
    public static function get(int $id, ?string $table = null): ?self
    {
        ...
        return static::getInstance([$table, $id], static fn () => static::query($table)->findId($id));
    }
    ...
    public static function require(int $id, ?string $table = null): self
    {
        ...
        if (!$dataset) {
            ...
            throw new RexRuntimeException('Dataset with ID "' . $id . '" does not exist in "' . $table . '"');
        }
    ...
    public static function getRaw(int $id, ?string $table = null): self
    {
        ...
        /* @phpstan-ignore-next-line */
        return static::getInstance([$table, $id], static function () use ($table, $id) {
            $class = self::tableToModel($table);
    ...
    {
        if (null === $this->id) {
            throw new RexRuntimeException('Calling getId() on new, non-existing datasets is not allowed, check existence before by $dataset->exists()');
        }
    ...
    public function getRelatedDataset(string $key): ?self
    {
        ...
        // php-cs-fixer would replace `Dataset::get()` by `self::get()`
    ...
    {
        if (!Extension::dispatch(new ExtensionPoint('YFORM_DATA_DELETE', true, ['table' => $this->getTable(), 'data_id' => $this->id, 'data' => $this]))) {
            return false;
        }
        ...
        Extension::dispatch(new ExtensionPoint('YFORM_DATA_DELETED', '', ['table' => $this->getTable(), 'data_id' => $this->id, 'data' => $this]));
    ...
    /**
     * @param null|callable(YForm):void $afterFieldsExecuted
     */
    public function executeForm(YForm $yform, ?callable $afterFieldsExecuted = null): string
    {
        ...
        if ($exists) {
            /** @var YForm $yform */
            $yform = Extension::dispatch(new ExtensionPoint('YFORM_DATA_UPDATE', $yform, ['table' => $this->getTable(), 'data_id' => $this->id, 'data' => $this]));
        } else {
            /** @var YForm $yform */
            $yform = Extension::dispatch(new ExtensionPoint('YFORM_DATA_ADD', $yform, ['table' => $this->getTable(), 'data' => $this]));
        }
        ...
        if (!$this->id) {
            Extension::register('YFORM_SAVED', function (ExtensionPoint $ep) {
                if ($ep->subject instanceof Exception) {
                    return;
                }
        ...
        if ($yform->objparams['actions_executed']) {
            if ($exists) {
                Extension::dispatch(new ExtensionPoint('YFORM_DATA_UPDATED', $yform, ['table' => $this->getTable(), 'data_id' => $this->id, 'data' => $this, 'old_data' => $oldData]));
            } else {
                Extension::dispatch(new ExtensionPoint('YFORM_DATA_ADDED', $yform, ['table' => $this->getTable(), 'data_id' => $this->id, 'data' => $this]));
            }
        }
    ...
    public function makeSnapshot(string $action): void
    {
        ...
        $user = Core::getEnvironment()->value;
        if ('backend' === $user && $rexUser = Core::getUser()) {
            $user = $rexUser->login;
        }
        // ep to overwrite user
        $user = Extension::dispatch(new ExtensionPoint('YCOM_HISTORY_USER', $user));
        ...
        foreach ($this->getFields() as $field) {
            ...
            if ($fieldObject instanceof BackendManagerRelation && !$value) {
        ...
        $sql->setQuery('INSERT INTO ' . Core::getTable('yform_history_field') . ' (`history_id`, `field`, `value`) VALUES ' . implode(', ', $inserts));
    }
    ...
    public function restoreSnapshot(int $snapshotId): bool
    {
        ...
        $sql->setDebug(self::$debug);
        $values = [];
        foreach ($sql->getArray(sprintf('SELECT * FROM %s WHERE history_id = %d', Core::getTable('yform_history_field'), $snapshotId)) as $value) {
    ...
    private function createForm(): YForm
    {
        ...
        foreach ($fields as $field) {
            /** @var class-string<AbstractBase> $class */
