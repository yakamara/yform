...
/**
 * @template T of Dataset
 * @extends SplFixedArray<T>
 *
 * @method Dataset offsetGet($offset)
    ...
    public function setData(array $data): self
    {
        foreach ($data as $dataset) {
            if ($dataset->getTableName() !== $this->table) {
                throw new InvalidArgumentException(sprintf('$data has to be an array of Dataset objects of table "%s", found dataset of table "%s".', $this->table, $dataset->getTableName()));
            }
        }
        ...
        foreach ($this->getTable()->getFields() as $field) {
            ...
            if ($useCheckbox) {
                $yform->setValueField('checkbox', [
                    ...
                    'label' => I18n::msg('yform_manager_multi_edit_field', $field->getLabel()),
                    'default' => $default,
                    'no_db' => true,
            ...
            if (!$this->save()) {
                foreach ($this as $dataset) {
                    foreach ($dataset->getMessages() as $message) {
                        $yform->objparams['warning_messages'][] = I18n::msg('yform_data') . ' ID ' . $dataset->getId() . ': ' . $message;
                    }
                }
            }
        }
