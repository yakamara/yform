...
class Api
{
    ...
    public static array $table_fields = ['status', 'name', 'description', 'table_icon', 'list_amount', 'list_sortfield', 'list_sortorder', 'prio', 'search', 'hidden', 'export', 'import', 'schema_overwrite', 'history', 'mass_deletion', 'mass_edit'];
    ...
    public static function removeTable(string $table_name): void
    {
        ...
        $f->setQuery('delete from ' . Field::table() . ' where table_name=:table_name', [':table_name' => $table_name]);
    ...
    {
        $columns = Sql::showColumns($table_name);
    ...
    public static function createMissingFieldColumns(array $field): void
    {
        ...
        if (!isset(self::$cacheColumnsByTable[$table_name])) {
            foreach (Sql::showColumns($table_name) as $column) {
        ...
        foreach ($field as $fieldKey => $fieldValue) {
            if (!isset(self::$cacheColumnsByTable[$table_name][$fieldKey])) {
                ...
                $alter->setQuery('ALTER TABLE `' . Field::table() . '` ADD `' . $fieldKey . '` TEXT NULL');
    ...
    public static function generateTableAndFields(Table $table, bool $delete_old = false): void
    {
        ...
        $EnsureTable = DbTable::get($table->getTableName());
            ...
            if ('value' == $field->getType()) {
                ...
                if ('none' != $db_type && '' != $db_type) {
                    ...
                    if (!$existingColumn || $table->overwriteSchema()) {
                        $EnsureTable
                            ->ensureColumn(new Column($field->getName(), $db_type, $field->getDatabaseFieldNull(), $default));
                    }
                }
            }
        }
