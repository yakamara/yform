...
/**
 * Tests for Api — the public write API for table
...
final class TablesSuite extends AbstractTestSuite
{
    ...
    public function testSetTableUpdatesExistingRow(): void
    {
        ...
        Table::deleteCache();
        $reloaded = Table::get($table->getTableName());
    ...
    public function testSetTableFieldInsertsNewValueField(): void
    {
        ...
        DbTable::get($table->getTableName())
            ->ensureColumn(new Column('title', 'varchar(255)', true))
            ->ensure();

        Api::setTableField($table->getTableName(), [
            'type_id' => 'value',
            'type_name' => 'text',
            'name' => 'title',
            'label' => 'Titel',
        ...
        Table::deleteCache();
        $reloaded = Table::require($table->getTableName());
    ...
    public function testRemoveTableDropsAllFields(): void
    {
        ...
        $this->assertNull(Table::get($tableName));

        $sql = Sql::factory();
        $rows = $sql->getArray(
            'SELECT COUNT(*) AS c FROM ' . Field::table() . ' WHERE table_name = :t',
            [':t' => $tableName],
        );
    ...
    public function testExportTablesetsReturnsValidJsonStructure(): void
    {
        ...
        $json = Api::exportTablesets([$table->getTableName()]);
    ...
    public function testImportTablesetsWithMalformedStructureThrows(): void
    {
        $this->assertThrows(
            Throwable::class,
            static fn () => Api::importTablesets('[{"no_table_or_fields":1}]'),
        );
    }
    ...
    public function testImportTablesetsWithInvalidJsonThrows(): void
    {
        ...
        $this->assertThrows(
            Throwable::class,
            static fn () => Api::importTablesets('not-json-at-all'),
        );
        $this->assertThrows(
            Throwable::class,
            static fn () => Api::importTablesets('"just-a-string"'),
        );
    }

    public function testRemoveTableCleansUpOrphanFieldRows(): void
    {
        // Issue #1575: Table::getFields() silently skips
        ...
        $remaining = Sql::factory()->getArray(
            'SELECT COUNT(*) AS c FROM ' . Field::table() . ' WHERE table_name = :t',
            [':t' => $tableName],
        );
    ...
    public function testReImportPreservesLocalFieldPrio(): void
    {
        ...
        $sql = Sql::factory();
        $sql->setQuery(
            'UPDATE ' . Field::table() . ' SET prio = CASE name WHEN :a THEN 10 WHEN :b THEN 5 WHEN :c THEN 20 END WHERE table_name = :t',
            [':a' => 'a', ':b' => 'b', ':c' => 'c', ':t' => $tableName],
        );
        Table::deleteCache();

        // Re-import the original tableset (with original prios 1/2/3).
        $json = (string) Api::exportTablesets([$tableName]);
        Api::importTablesets($json);
        Table::deleteCache();

        $imported = Table::require($tableName);
        $byName = [];
    ...
    public function testExportImportRoundtripPreservesFields(): void
    {
        ...
        $json = Api::exportTablesets([$tableName]);
        ...
        Api::importTablesets((string) $json);
        Table::deleteCache();
    ...
    public function testImportTablesetsUnderRenamedTablePreservesFields(): void
    {
        ...
        $json = (string) Api::exportTablesets([$sourceName]);
        ...
        Api::importTablesets((string) $rewritten);
        Table::deleteCache();

        $imported = Table::require($renamed);
