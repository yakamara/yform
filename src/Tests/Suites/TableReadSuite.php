...
/**
 * Tests for the read-only API of Table.
...
final class TableReadSuite extends AbstractTestSuite
{
    ...
    {
        // Issue #1548: Table is no longer final, community
        // addons (e.g. external-DB managers via YOrm) can subclass it.
        $reflection = new ReflectionClass(Table::class);
        $this->assertFalse($reflection->isFinal(), 'Table must be subclassable.');
    }
    ...
    {
        $this->assertNull(Table::get('this_table_should_never_exist_' . uniqid()));
    }
    ...
    public function testRequireThrowsForUnknownTable(): void
    {
        ...
        $this->assertThrows(
            Exception::class,
            static fn () => Table::require($missing),
        );
    }
    ...
    public function testHasHistoryReflectsFlag(): void
    {
        ...
        $on = Table::require($withHistory->getTableName());
        $off = Table::require($noHistory->getTableName());
    ...
    public function testSetTableApiPropagatesHistoryMassFlags(): void
    {
        ...
        DbTable::get($name)->ensurePrimaryIdColumn()->ensure();
        Api::setTable([
            'table_name' => $name,
            'name' => 'Mass Flags',
            'status' => 1,
        ...
        $row = Sql::factory();
        $row->setQuery(
            'SELECT history, mass_deletion, mass_edit FROM ' . Table::table() . ' WHERE table_name = :n',
            [':n' => $name],
        );
    ...
    public function testGetListAmountFallsBackTo100WhenZero(): void
    {
        ...
        // Table::getListAmount() falls back to 100 when < 1.
