...
class YForm
{
    ...
    public function __construct(array $params = [])
    {
        ...
        $this->objparams['csrf_protection_error_message'] = Core::isFrontend() ? '{{ csrf.error }}' : I18n::msg('csrf_token_invalid');
        ...
        $this->objparams['sql_object'] = null; // Sql
        ...
        Extension::dispatch(new ExtensionPoint('YFORM_INIT', $this));
    }
    ...
    public function setRedaxoVars($aid = '', $clang = '', $params = []): self
    {
        ...
        if ('' == $aid) {
            $aid = Article::getCurrentId();
        }

        // Url::article() is typed ?int for both ids, while callers routinely pass strings
        // (REX_ARTICLE_ID expands to one) — without the casts that is a TypeError.
        $this->setObjectparams('form_action', Url::article((int) $aid, (int) $clang, $params));
        return $this;
    }
    ...
    {
        Extension::dispatch(new ExtensionPoint('YFORM_GENERATE', $this));
        $this->executeFields();
    ...
    public function executeFields(): self
    {
        ...
        Extension::dispatch(new ExtensionPoint('YFORM_EXECUTE_FIELDS', $this));
        ...
        if ($this->objparams['getdata']) {
            if (!$this->objparams['sql_object'] instanceof Sql) {
                $this->objparams['sql_object'] = Sql::factory();
    ...
    public function initializeFields(): self
    {
        ...
        for ($i = 0; $i < $rows; ++$i) {
            ...
            if ('validate' == $element[0]) {
                $class = FieldRegistry::getClass('validate', trim($element[1]));
                $type = 'validates';
            } elseif ('action' == $element[0]) {
                $class = FieldRegistry::getClass('action', trim($element[1]));
                $type = 'actions';
            } else {
                $class = FieldRegistry::getClass('value', trim($element[0]));
                $type = 'values';
            }

            // An unknown type is rendered as raw html, exactly as in REDAXO 5: the line is re-read as an
            // `html` element with the original tokens as its content.
            if (null === $class) {
    ...
    public function executeActions()
    {
        ...
        if ($this->objparams['form_show']) {
            ...
            $this->objparams['output'] .= $this->parse('form');
        }
    ...
    /**
     * Renders one of yform's form fragments.
     *
     * `$template` is a fragment key, not a path ("form", "errors"). The backend or
     * frontend variant is chosen from the current context; the fragment gets this
     * form object as `$yform` plus everything in `$params`.
     *
    ...
    {
        return YFormFragment::render($template, 'form', ['yform' => $this] + $params);
    }
    ...
    public static function showHelp(): string
    {
        ...
        foreach ($classesDescription as $title => $content) {
            ...
            $fragment->setVar('title', I18n::msg('yform_' . $title));
    ...
    public static function getTypeArray(): array
    {
        ...
        foreach (self::getTypes() as $arr_key) {
            foreach (FieldRegistry::getClasses($arr_key) as $class) {
                /** @var AbstractBase $class */
                $class = new $class();
