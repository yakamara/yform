...
$list = \Yakamara\YForm\ListView\YList::factory($query, $table->getListAmount());

$list->addTableAttribute('class', 'table-striped table-hover yform-table-' . \Redaxo\Core\Util\Str::normalize($this->table->getTableName()));

$rex_yform_list[$list->getPager()->getCursorName()] = \Redaxo\Core\Http\Request::request($list->getPager()->getCursorName(), 'int', 0);

if ($hasDataPageFunctions('add') && $this->table->isGranted('EDIT', \Redaxo\Core\Core::getUser())) {
    $thIcon = '<a class="rex-link-expanded" href="index.php?' . http_build_query(array_merge(['func' => 'add'], $rex_link_vars)) . '"' . \Redaxo\Core\Core::getAccesskey(\Redaxo\Core\Translation\I18n::msg('add'), 'add') . '><i class="rex-icon rex-icon-add"></i></a>';
    $tdIcon = '<i class="rex-icon rex-icon-editmode"></i>';
    $list->addColumn($thIcon, $tdIcon, 0, ['<th class="rex-table-icon">###VALUE###</th>', '<td class="rex-table-icon" data-title="' . \Redaxo\Core\Translation\I18n::msg('id') . '">###VALUE###</td>']);
...
$list->setColumnLabel('id', \Redaxo\Core\Translation\I18n::msg('yform_id'));
...
foreach ($this->table->getFields() as $field) {
    if (!$field->isHiddenInList() && $field->getTypeName()) {
        $listValueClass = \Yakamara\YForm\FieldRegistry::getClass('value', $field->getTypeName());
        if (null !== $listValueClass && method_exists($listValueClass, 'getListValue')) {
            $list->setColumnFormat(
                $field->getName(),
                'custom',
                [$listValueClass, 'getListValue'],
                ['field' => $field->toArray(), 'fields' => $this->table->getFields()],
            );
        }
        $list->setColumnLayout($field->getName(), ['<th>###VALUE###</th>', '<td class="yform-table-field-value-' . \Redaxo\Core\View\escape($field->getTypeName()) . ' yform-table-field-name-' . \Redaxo\Core\View\escape($field->getName()) . '" data-title="###LABEL###">###VALUE###</td>']);
    }
...
if (isset($rex_yform_manager_opener['id'])) {
    $list->addColumn(\Redaxo\Core\Translation\I18n::msg('yform_function'), '');
    $list->setColumnFormat(
        \Redaxo\Core\Translation\I18n::msg('yform_function'),
        'custom',
        static function ($params) {
            ...
            return '<span class="yform-dataset-widget"><a
                ...
                data-value="' . \Redaxo\Core\View\escape($value, 'html') . ' [id=###id###]"
                data-multiple="' . $params['params']['opener_multiple'] . '">' . \Redaxo\Core\Translation\I18n::msg('yform_data_select') . '</a></span>';
        },
        [
    ...
    foreach ($actionButtonsLoaded as $actionButtonKey => $actionButton) {
        ...
        $actionButtons[$actionButtonKey] = '<a ' . \Redaxo\Core\Util\Str::buildAttributes($attributes) . '>' . $actionButton['content'] . '</a>';
    }

    $fragment = new \Redaxo\Core\View\Fragment();
    $fragment->setVar('buttons', $actionButtons, false);
    $buttons = $fragment->parse('yform/manager/action_buttons.php');

    $list->addColumn(\Redaxo\Core\Translation\I18n::msg('yform_function') . ' ', $buttons);
}

$list->setColumnLayout(\Redaxo\Core\Translation\I18n::msg('yform_function') . ' ', ['<th class="rex-table-action" colspan="' . $colspan . '">###VALUE###</th>', '<td class="rex-table-action">###VALUE###</td>']);

$list = \Redaxo\Core\ExtensionPoint\Extension::dispatch(new \Redaxo\Core\ExtensionPoint\ExtensionPoint('YFORM_DATA_LIST', $list, ['table' => $this->table]));

$fragment = new \Redaxo\Core\View\Fragment();
$fragment->setVar('title', \Redaxo\Core\Translation\I18n::msg('yform_tabledata_overview'));
